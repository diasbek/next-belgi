...
function applyClassificationToReport(
  ...
  const queryClasses = parseQueryClassNumbers(niceClasses);
  const scored = scoreConflictFromSources(report.sources, queryClasses);
  const assessment = scoreAssessmentFromSources(report.sources, queryClasses);
  const classRisks =
    scored.classRisks.filter((c) => c.classNumber > 0).length > 0
      ? scored.classRisks.filter((c) => c.classNumber > 0)
      : queryClasses.slice(0, 3).map((classNumber) => ({
          classNumber,
...
function normalizeUpstream(
  ...
  // Recompute conflict + assessment when upstream omitted them
  const needsRescore =
    !Array.isArray(data.classRisks) ||
    data.classRisks.length === 0 ||
    !data.assessment;

  if (needsRescore) {
    const queryClasses = parseQueryClassNumbers(merged.niceClasses);
    const scored = scoreConflictFromSources(merged.sources, queryClasses);
    const assessment = scoreAssessmentFromSources(
      merged.sources,
      queryClasses,
    );
